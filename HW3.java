...
public class HW3 {
    ...
    public ArrayList<Countries> countriesAL = new ArrayList<Countries>();         // linked list of countries
    ...
    HW3() { // this constructor is responsible for calling all methods and determining if the user wants to continue
        try {
            create();
            assignBorders();
            do {
    ...
    public void create() { // this method creates a linked list and an array of countries
        ...
        try {
            ...
            for (int i = 0; i < numberOfCountries; i++) { // fills the linked list with countries
                countriesAL.add(countriesArray[i]);
            } // end for
    ...
    public void assignBorders() { // this method assigns borders to each country
        ...
        for (int i = 0; i < numberOfCountries; i++) { // loop through all the countries
            ...
            String borderNumString = Integer.toString(countriesAL.get(i).borders.borderNum);
            ...
            for (int k = 0; k < borderNumString.length(); k++) { // loop through the borderNum
                for (int j = 0; j < numberOfCountries; j++) { // loop through all the countries
                    ...
                    String indexString = Integer.toString(countriesAL.get(j).borders.countryIndex);
                    ...
                    if (borderNumString.charAt(k) == indexString.charAt(0)) {
                        ...
                        borderArray[k] = countriesAL.get(j).countryName;
                    } // end if
                } // end for
            } // end for
            ...
            countriesAL.get(i).borders.borderString = builder.toString(); // set the current country's border list to the string
        } // end for
    } // end assignBorders
    ...
    public void importData() { // this method displays the data for all countries
        ...
        for (int i = 0; i < numberOfCountries; i++) {
            System.out.println("Country Name: " + countriesAL.get(i).countryName);
            System.out.println("Latitude: " + countriesAL.get(i).latitude);
            System.out.println("Longitude: " + countriesAL.get(i).longitude);
            System.out.println("Country Area: " + countriesAL.get(i).countryArea);
            System.out.println("Country Population: " + countriesAL.get(i).countryPopulation);
            System.out.println("Country GDP: " + countriesAL.get(i).countryGDP + "*10^12");
            System.out.println("Country Year: " + countriesAL.get(i).countryYear + "\n");
        } // end for
    } // end importData
    ...
    public void showBordersForGermany() { // this method displays the borders for germany
        ...
        for (int i = 0; i < numberOfCountries; i++) { // loop through the countries
            if (countriesAL.get(i).countryName.equals("Germany")) { // if the country is germany
                System.out.println(countriesAL.get(i).countryName + ":");
                System.out.print(countriesAL.get(i).borders.borderString); // print the borders
            } // end if
        } // end for
    ...
    public void showAllBorders() { // this method displays borders for a country of the user's choice
        ...
        try {
            ...
            for (int i = 0; i < numberOfCountries; i++) { // loop through the countries
                if (countriesAL.get(i).borders.countryIndex == chooseBorder) { // if the input matches a country's index
                    System.out.println(countriesAL.get(i).countryName + ":");
                    System.out.print(countriesAL.get(i).borders.borderString); // print the borders
                } // end if
            } // end for
    ...
    public void showPopulations() { // this method displays countrys with a population over 35 million
        ...
        for (int i = 0; i < numberOfCountries; i++) {
            if (countriesAL.get(i).countryPopulation > maxPopulation) {
    ...
    public void showBordersAndPopulation() {
        ...
        System.out.println("Here is a list of all countries that border Germany and have a population of over 35 " +
                "million: ");
        for (int i = 0; i < numberOfCountries; i++) {
            if (countriesAL.get(i).borders.countryIndex != 0 && countriesAL.get(i).countryPopulation > maxPopulation) {
                System.out.print(countriesAL.get(i).countryName);
            } // end if
        } // end for
